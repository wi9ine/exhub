...
- packages/spec 폴더의 각 거래소별 spec 파일은 공식 문서를 기반으로 작성한다.
- packages/spec 폴더의 각 spec이 전체 프로젝트의 SSOT이다. 즉, 스펙 변경이 필요한 경우 packages/spec 폴더의 spec을 먼저 변경하고 `pnpm generate`를 통해 거래소별 코드들을 갱신한다.
