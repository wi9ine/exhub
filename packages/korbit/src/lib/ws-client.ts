...
import type { KorbitWsClient, KorbitWsClientOptions } from "./ws-types";

export function createKorbitWsClient(options: KorbitWsClientOptions = {}): KorbitWsClient {
  ...
  return {
    ...
    subscribeMyTrade: (options, onMessage) =>
      getPrivateManager().subscribe("myTrade", options, onMessage as (data: unknown) => void),
