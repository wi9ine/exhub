...
| X-COINONE-SIGNATURE | Required           | HMAC(X-COINONE-PAYLOAD, SECRET\_KEY, SHA512).hexdigest() |
...
| fee\_rates         | Array\[Object] | 배열 형태의 수수료 정보                                   |
| - quote\_currency  | String         | 마켓 기준 통화 (예: "KRW")                             |
| - target\_currency | String         | 수수료 조회하려는 종목의 심볼 (예: "BTC")                     |
| - maker            | NumberString   | makers 수수료율 (즉시 체결되지 않고 오더북에 쌓이는 주문, 예: "0.001) |
| - taker            | NumberString   | takers 수수료율 (즉시 체결되는 주문, :예: "0.001")           |
...
```json
{
  ...
  "paths": {
    "/v2.1/account/trade_fee": {
      "post": {
        "summary": "전체 수수료 조회",
        "description": "거래 가능한 모든 종목의 수수료 조회",
        "operationId": "find-all-trade-fees",
        "parameters": [
          {
  ...
  "_id": "68394678760f3c0030350497:68394678760f3c00303504a2"
}
```