...
| currency      | string | optional          | 출금 주소를 조회 하려는 종목                                       |
...
| withdrawal\_addresses | Array\[Object]    | 배열 형태의 출금 주소 목록                         |
| - currency            | String            | 출금 주소로 등록된 가상자산 종목                      |
| - address             | String            | 출금 주소                                   |
| - secondary\_address  | String (Nullable) | 추가적인 출금 주소 정보 (Memo 혹은 Destination Tag) |
| - nickname            | String            | 주소의 별명 (사이트에 주소 등록 시 고객이 설정한 이름)        |
| - created\_at         | Number            | 출금 주소를 등록한 시각 (UNIX  timestamp)         |
...
```json
{
  ...
  "paths": {
    "/v2.1/transaction/coin/withdrawal/address_book": {
      "post": {
        "summary": "출금 주소 목록 조회",
        "description": "2차 인증 및 수취인정보 등록이 완료된 주소 조회. (해당 주소로만 API출금이 가능합니다)",
        "operationId": "coin-withdrawal-address-book",
        "parameters": [
          {
  ...
  "_id": "68394678760f3c0030350497:68394678760f3c00303504f2"
}
```