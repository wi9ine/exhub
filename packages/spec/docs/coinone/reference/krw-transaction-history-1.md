...
| errorCode      | NumberString   | error 발생 시 에화러코드 반환, 성공인 경우 0 반환  |
| krwHistory     | Array\[Object] | 배열 형태의 원 입출금 내역 정보 목록             |
| - bankCode     | Number         | 은행 코드 (아래 bankCode표 참고)           |
| - depositor    | String         | 입금 계좌 소유자 명                       |
| - amount       | NumberString   | 입/출금 수량                           |
| - processLevel | Number         | 원화 입/출금 단계 (아래 processLevel 표 참고) |
...
```json
{
  ...
  "paths": {
    "/v2/transaction/krw/history": {
      "post": {
        ...
        "description": "원화의 입금, 출금 내역을 조회",
        "operationId": "krw-transaction-history-1",
        "parameters": [
          {
            "name": "X-COINONE-PAYLOAD",
            "in": "header",
            "description": "Parameter object -> JSON string -> base64",
            "required": true,
            "schema": {
              "type": "string"
            }
  ...
  "_id": "68394678760f3c0030350497:68394678760f3c00303504c3"
}
```